namespace LittleHeroes {
	...
	public class ItemsColorPanel : ABasicPanel {
		...
		private List<UIItemColor>	_items;
		...
		public override float hidePanel ( ) {
			...
			uint count = 0;
			foreach ( UIItemColor item in _items ) {
		...
		private void updateItems ( ) {
			...
			if ( _version < version ) {
				...
				foreach ( XmlNode item in item_root ) {
					...
					UIItemColor	itemCpt	= itemObj.GetComponent<UIItemColor> ( );
